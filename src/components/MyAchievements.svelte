<script>
	...
	let write_achievements = [
		...
		{
			name: 'Написано 100 казок!',
			value: 100,
			path: '/img/K_hundred.png',
			pathg: '/img/gK_hundred.png'
		}
	];
	...
	let like_achievements = [
		...
		{
			name: 'Зібрано 100 лайків!',
			value: 100,
			path: '/img/L_hundred.png',
			pathg: '/img/gL_hundred.png'
		}
	];

	let likes_total = getKazkyLikesTotal(user_kazky);
</script>
	...
	<div class="achieves">
		{#each write_achievements as achv}
			<div class="achiev-unit">
				{#if user_kazky.length >= achv.value}
